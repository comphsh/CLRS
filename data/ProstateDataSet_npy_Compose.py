...
class ProstateDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(16, 128, 128), scale=True, mirror=True, ignore_label=255, output_class_num=1, proj_path=''):
        ...
        self.train_transforms.append(RandomCrop3D_Circle1(output_size=crop_size, scale=True))
        ...
        self.img_ids= []
        for dirname in open(os.path.join(proj_path, self.list_path)): #打开txt文件，逐行读取患者名称
        ...
        for item in self.img_ids:
            ...
            elif output_class_num == 3:
                label_file = filepath + '_seg_onehot_has_background.npy'
            else:
...
class ProstateValDataSet(data.Dataset):
