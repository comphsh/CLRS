...
if __name__ == '__main__':
    ...
    model = VNet(n_channels=4, n_classes=3, normalization='batchnorm', has_dropout=False)
    with torch.cuda.device(0):
        macs, params = get_model_complexity_info(model, (4, 112, 112, 80), as_strings=True,
                                                 print_per_layer_stat=True, verbose=True)
        print(macs , params)
        print('{:<30}  {:<8}'.format('Computational complexity: ', macs))
        print('{:<30}  {:<8}'.format('Number of parameters: ', params))
    with torch.cuda.device(0):
        macs, params = get_model_complexity_info(model, (4, 96, 96, 96), as_strings=True,
                                                 print_per_layer_stat=True, verbose=True)
        print('{:<30}  {:<8}'.format('Computational complexity: ', macs))
        print('{:<30}  {:<8}'.format('Number of parameters: ', params))
    import ipdb;
    data = torch.randn(1,4,80,160,160)
    pred = model(data)
